...
EditWidgetVarField::EditWidgetVarField(QString fieldName,MacrorVarEditWindow* parentW ):
    QWidget(parentW),
    pw(parentW),
    name(fieldName),
    v(parentW->getFieldClass(fieldName)),
...
EditWidgetVarField* EditWidgetVarField::create(QString fieldName,MacrorVarEditWindow* parentW)
{
    const Markov_IO::ABC_Saveable* v=parentW->getFieldClass(fieldName);
    if (v)
        return new EditWidgetVarField(fieldName,parentW);
    else
    {
        const Markov_IO::ABC_Object* o=parentW->getFieldObject(fieldName);
        if (o->myClass()==Markov_IO::Object<Markov_LA::M_Matrix<double> >::ClassName())
...
void EditWidgetVarField::updateValue()
{
    if (type==BOOL||type==SIZE||type==DOUBLE||type==STRING)
    {
        pw->replaceField(name,lineEdit->displayText());
...
void EditWidgetVarField::editMe()
{
    MacrorVarEditWindow* ew=new MacrorVarEditWindow(name,pw);
    ew->show();
}
