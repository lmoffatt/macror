...
namespace Markov_Bay
{
  ...
  SimpleOptimization::SimpleOptimization(Markov_IO::ABC_Environment* e,
    ...
    name_(name),
    par_(),
    ML_(Markov_Likelihood(e,P,E,O)),
    O_(O),
    maxfeval_(O.count("Maximum_number_of_evaluations")),
    maxiter_(O.count("Maximum_number_of_iterations")),
    io_(io)
  {
    Markov_Mol::ABC_PatchModel* patch=dynamic_cast<Markov_Mol::ABC_PatchModel*>(e->getVar(P));
    if (patch!=nullptr)
      par_=patch->get_parameters();
