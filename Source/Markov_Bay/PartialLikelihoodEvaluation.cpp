 ...
 Markov_IO::ClassDescription PartialLikelihoodEvaluation::GetDescription()const
{
	...
	Markov_IO::ClassDescription desc(myClass(),mySuperClass());	desc.push_back("name",myName());
...

 {
     ...
     *this=PartialLikelihoodEvaluation(name,
				       Log_Likelihood,
 ...
 PartialLikelihoodEvaluation::PartialLikelihoodEvaluation(const std::string& name,
			     double Log_Likelihood,
			     double expectedLogLikelihood,
			     double s2logLikelihood,
			     const Markov_LA::M_Matrix<double>& PartialLogL):
     LikelihoodEvaluation(name,
	 Log_Likelihood,
