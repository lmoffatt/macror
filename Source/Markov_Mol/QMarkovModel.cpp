...

{
  ...
  {
        std::string kij="k_"+Markov_IO_New::ToString(i)+
            "_to_"+Markov_IO_New::ToString(j);
        return kij;
 }


  /// constructor
  Q_Markov_Model::Q_Markov_Model(const M_Matrix<double>& Q_matrix,
                                 const M_Matrix<double>& conductance_vector,
                                 const M_Matrix<std::size_t>& agonist_vector,
                                 double unitary_conductance,
                                 const ABC_PatchModel* mypatch):
    patch_(mypatch),
    k_u(nrows(Q_matrix)),
    Q_M(Q_matrix),
  ...
  bool Q_Markov_Model::get_states_from_kij_Label(std::string kij,std::size_t& i,std::size_t& j)
  {
    ...
    if (!Markov_IO_New::ToValue<std::size_t>(s,i))
      return false;
    s=kij.substr(pos+4);
    if (!Markov_IO_New::ToValue<std::size_t>(s,j))
      return false;
    return true;
  }
...
//    push_backRefMethod("conductance_vector",
//                       &Q_Markov_Model::g,{},
//                       "",
...
//    push_backRefMethod("agonist_vector",
//                       &Q_Markov_Model::n_bound_agonists,{},
//                       "",
...
//    push_backValMethod("unitary_conductance",
//                       &Q_Markov_Model::gamma,{},
//                       "",
...
//  {
//    parameters_Map=Markov_IO_New::Parameters(id()+"_parameters");
//    parameters_Map.push_back(gamma_Label(),gamma_d, Markov_IO_New::Parameters::LOG);
...
//  int Q_Markov_Model::apply_parameters(const Markov_IO_New::Parameters& beta)//
//  {
//    int numPar=0;
//    bool noone=true;
...
//  // takes new parameters
  ...
  Q_Markov_Model::Q_Markov_Model(const Q_Markov_Model& other):
    Markov_IO_New::Implements_Class_Reflection<Q_Markov_Model>(other),
    patch_(other.patch_),
    name_(other.name_),
    k_u(other.k_u),
    Q_M(other.Q_M),  //par
  ...
  Q_Markov_Model::Q_Markov_Model(Q_Markov_Model&& other):
    Markov_IO_New::Implements_Class_Reflection<Q_Markov_Model>(std::move(other)),
    patch_(std::move(other.patch_)),
    name_(std::move(other.name_)),
    k_u(std::move(other.k_u)),
...
*/
  ...
  Markov_Transition_step Q_Markov_Model::Q_dt (
      const Markov_IO_New::ABC_measure_point& xdt_A,
      bool is_averaging,
      bool varying_x,
      bool two_anchor) const
  {
    Markov_IO_New::x_dt xdt(xdt_A);
  ...
  Markov_Transition_step Q_Markov_Model::get_Q_dt_no_avg(
      const Markov_IO_New::ABC_measure_point& xdt_A) const
  {
    Markov_IO_New::x_dt xdt(xdt_A);
  ...
  Markov_Transition_step& Q_Markov_Model::get_Q_dt_avg_var(const Markov_IO_New::ABC_measure_point& xdt_A, Markov_Transition_step& Qdt) const
  {
    x_dt xdt(xdt_A);
    // build EE0
  ...
  Markov_Transition_step& Q_Markov_Model::get_Q_dt_two_anchor(const Markov_IO_New::ABC_measure_point& xdt_A, Markov_Transition_step& Qdt) const
  {
    x_dt xdt(xdt_A);
    // build EE0
    const Markov_Transition_rate Qx=Q_x(xdt.x(),true);
 ...
 */
  ...
  Markov_Transition_step Q_Markov_Model::Q_step (const Markov_IO_New::ABC_measure_step& xdt_A,
                                                 bool is_averaging,
                                                 bool two_anchor)const
  {
...
//  bool Q_Markov_Model::loadFromObjectValue(const Markov_IO_New::ABC_Value *source)
//  {
...
//        decltype(a_M) a;
//        double gamma;
//        bool bQ=source->getVal("Q_matrix",Q);
//        auto bg=source->getVal("conductance_vector",g);
//        bool ba=source->getVal("agonist_vector",a);
//        bool bgamma=source->getVal("unitary_conductance", gamma);

//        if (bQ&&bg&&ba&&bgamma)
//          {
//            *this=Q_Markov_Model(source->id(),source->myVar(),Q,g,a,gamma);
//            return true;
//          }
//        return false;
//      }
...
//  //std::ostream& Q_Markov_Model::put(std::ostream&s) const
  ...
  //{
  //    return Markov_IO_New::ABC_Saveable::FileName();
  //}
...
//  Q_Markov_Scheme *Q_Markov_Scheme::toSameVar(const ABC_Value *source) const
//  {
//    if (source->myClass()!=myClass())
//      return nullptr;
//    decltype(g_) gt;
//    decltype(a_) at;
//    std::map<std::size_t,std::set<std::size_t>> ug;
//    auto bg=source->getVal(Q_Markov_Model::g_Label(),gt);

//    bool ba=source->getVal(Q_Markov_Model::a_Label(),at);
//    bool bug=source->getVal(connections_Label(), ug);

//    if (bg&&ba&&bug)
//      {

//         return new Q_Markov_Scheme(source->id(),myClass(),ug,gt,at,source->Tip(),source->WhatThis());

//        }
//    else
//      {
//        putErrorOut("toSameVar failed. " + source->toString());
//      return nullptr;
//      }
//  }

//  Implements_Complex_Value *Q_Markov_Scheme::to_PlainValue() const
//  {

//    auto c=new Markov_IO_New::Implements_Complex_Value(id(),myClass(),Tip(),WhatThis());
//    c->push_backVarRef(Q_Markov_Model::patch_Label(),p_->id(),p_->myClass());
//    c->push_backVal(Q_Markov_Model::g_Label(),g_);
//    c->push_backVal(Q_Markov_Model::a_Label(),a_);
//    c->push_backVal(connections_Label(),ug_.getMap());
//    return c;

//  }

//  Implements_Complex_Value *Q_Markov_Scheme::to_PlainValue(const ABC_Value *source) const
//  {
//    if ((source->myClass()!=Q_Markov_Model::ClassName()))
//      return nullptr;
//    else
//      {

//        auto* q=dynamic_cast<Q_Markov_Model const*>(source);

//        auto c=new Implements_Complex_Value(source->id(),id(),source->Tip(),source->WhatThis());

//        c->push_backVarRef("patch",q->patch()->id(),q->patch()->myClass()
//                           ,"Belongs to patch","the patch that contains this model");

//        c->push_backVal(q->gamma_Label(),q->gamma());

//        for (std::size_t i=0; i<q->k(); i++)
//          {
//            auto s=ug_.connectedTo(i);
//            for (auto j:s)
//              {
//                c->push_backVal(q->kij_Label(i,j),q->Q()(i,j));
//              };
//          }
//        return c;
...
namespace Markov_Mol
{
  ...
  Markov_IO::Implements_Complex_Value *Q_Markov_Model::to_PlainValue() const
  {
    auto c=new Implements_Complex_Value(id(),myVar(),Tip(),WhatThis());

    //c->push_backVarRef("patch",patch()->id(),patch()->myClass(),"Belongs to patch","the patch that contains //this model");
  ...
  Q_Markov_Model *Q_Markov_Model::toSameVar(const ABC_Value *source) const
  {
      decltype(Q0_M) Qt;
        decltype(g0_M) gt;
        decltype(a_M) at;
  ...
  */
  ...
  /// constructor
  Q_Markov_Model::Q_Markov_Model(const std::string& idName,
...
//  }
  ...
  void Q_Markov_Model::buildParameters()
  {
    parameters_Map=Markov_IO::Parameters(id()+"_parameters");
...
//  bool Q_Markov_Model::loadFromObjectValue(const Markov_IO::ABC_Value *source)
//  {
//    if (sameFields(source))
//      {
//        decltype(Q_M) Q;
...
//  //std::ostream& Q_Markov_Model::put(std::ostream&s) const
  ...
  */
  ...
  Implements_Complex_Value *Q_Markov_Scheme::to_PlainValue() const
  {
    ...
    auto c=new Markov_IO::Implements_Complex_Value(id(),myClass(),Tip(),WhatThis());
    c->push_backVarRef(Q_Markov_Model::patch_Label(),p_->id(),p_->myClass());
    c->push_backVal(Q_Markov_Model::g_Label(),g_);
    c->push_backVal(Q_Markov_Model::a_Label(),a_);
    c->push_backVal(connections_Label(),ug_.getMap());
