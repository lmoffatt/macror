...
namespace Markov_Mol
{
  ...
  {
    auto c=new Implements_Complex_Value(id(),myClass(),Tip(),WhatThis());

    c->push_backVarRef("patch",patch()->id(),patch()->myClass(),"Belongs to patch","the patch that contains this model");


    c->push_backVal(Q_Label(),Q());
    c->push_backVal(g_Label(),g());
  ...
  */
  ...
  Q_Markov_Model::Q_Markov_Model(const std::string& idName,
    ...
    Implements_Class_Reflection<Q_Markov_Model>(idName,idVarName,this,tip,whatthis),
    patch_(mypatch),
    k_u(nrows(Q_matrix)),
    Q_M(Q_matrix),
