...
namespace Markov_IO {
  ...
  Token_Buffer Implements_ValueId::toTokens() const
  {
    Token_Buffer out;
    if (!Tip().empty())
      out<<"# \""+Tip()+"\" \n";
...
*/
  ...
  bool Implements_ValueId::processTokens(Token_Buffer &tok)
  {
  ...
  Token_Buffer Implements_Complex_Value::toTokens() const
  {
    auto out=Implements_ValueId::toTokens();
    out<<"begin"<<"\n";
    for (std::size_t i=0; i<numChilds(); i++)
      {
        if (getChild(ith_ChildName(i))!=nullptr)
          out<<getChild(ith_ChildName(i))->toTokens();
      }
    out<<myVar()<<"end"<<"\n";
    return out;
  }

  bool Implements_Complex_Value::processTokens(Token_Buffer &t)
  {
...
*/
  ...
  ABC_Value *ABC_Value::getValueFromStream(Token_Buffer& t)
  {
    auto pos0=t.pos();
