...
namespace Markov_IO_New
{
  ...
  x_dt::x_dt(const ABC_measure_point& m):dt_d(m.dt()),x_d(m.x()),tolComp_(std::sqrt(std::numeric_limits<double>::epsilon()),
  ...
  x_dt::x_dt(double dt_,double x_):
    dt_d(dt_),
    x_d(x_),
    tolComp_(std::sqrt(std::numeric_limits<double>::epsilon()),
             std::sqrt(std::numeric_limits<double>::epsilon()))
  {}
