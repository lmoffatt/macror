...
ClassDescription Trace::GetDescription()const
{
    ClassDescription desc(myClass(),mySuperClass());;

    desc.push_back("name",name_);
    desc.push_back("evolution_of_x",tx_,
                   "\t\t[s]\t\t[uM/uM]",
                   "evolution of the agonist concentration",
                   "we indicate the times where a change in agonist concentration "
                   "occurs. It assumes a stepped change pattern, and we only indicate "
                   "the times where a concentration jump occurs");
    desc.push_back("evolution_of_y",
                   ty_,
                   "\t\t[s]\t[pA]",
                   "evolution of the recorded (or simulated) current",
                   "it is assumed that each value correspond for a period "
                   "of time that ends on the following measurement ");
    desc.push_back("trace_interval",
                   trace_interval_,
                   "[s]",
...
ClassDescription Trace::GetState()const
{
    return GetDescription();

}

bool Trace::LoadFromDescription(const ClassDescription& classDes)
{
    ...
    Markov_LA::M_Matrix<double> evolution_of_x;
    if (!ToValue(classDes["evolution_of_x"],evolution_of_x))
        return false;


    Markov_LA::M_Matrix<double> evolution_of_y;
    if (!ToValue(classDes["evolution_of_y"],
                 evolution_of_y))
        return false;


    double trace_interval;
    if (!ToValue(classDes["trace_interval"],
                 trace_interval))
        return false;



    *this=Trace(name,evolution_of_x,evolution_of_y,trace_interval);
