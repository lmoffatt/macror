...
namespace Markov_Mol
{
  ...
  public:
    ...
    virtual bool unload_ABC_Var()
    {
       if (!replaceValue("Q_matrix",
                       Q_M)) return false;
       if (!replaceValue("conductance_vector",
                       g0_M)) return false;
       if (! replaceValue("agonist_vector",
                       a_M)) return false;
       if (! replaceValue("unitary_conductance",
                       gamma_d)) return false;

       return true;

    }

    virtual ABC_Var* load_ABC_Var()
    {
